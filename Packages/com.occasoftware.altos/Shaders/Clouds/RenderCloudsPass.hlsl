...
half2 _CLOUD_WEATHERMAP_VELOCITY;
int _USE_CLOUD_WEATHERMAP_TEX;
half _CLOUD_WEATHERMAP_SCALE;
half2 _CLOUD_WEATHERMAP_VALUE_RANGE;
Texture2D _CLOUD_DENSITY_CURVE_TEX;
half3 _PLANET_CENTER;
...
float4 _ShadowCasterCameraPosition[4];
...
half2 GetWeathermapUV(half3 rayPosition, half3 rayOrigin)
{
...
half GetWeathermap(StaticMaterialData materialData, RayData rayData, half cloudinessAtPoint)
{
    ...
    {
        weathermapUV = GetWeathermapUV(rayData.rayPosition, materialData.mainCameraOrigin);
        weathermapSample = materialData.weathermapTex.SampleLevel(linear_repeat_sampler, weathermapUV, 0).r;
    }
    else
    {
        // To do: Move this to a separate shader, render to RT, then sample it
        // To do: Also use this for the Weathermap (e.g., Rain).
        weathermapUV = GetWeathermapUV(rayData.rayPosition, materialData.mainCameraOrigin);
...
OSLightingData GetLightingDataVolumetric(StaticMaterialData materialData, RayData rayData, AtmosphereData atmosData, int mip)
{
	...
	*/
    ...
    #define INV_LIGHT_SAMPLE_COUNT_PLUS_ONE 1.0 / 5.0
	...
	*/
    ...
    {
        half lightSample = half(i) * INV_LIGHT_SAMPLE_COUNT_PLUS_ONE;
        lightSample = lightSample * lightSample; // Simplified pow2
...
void SampleHighAltitudeClouds(RayData rayData, StaticMaterialData materialData, AtmosphereData atmosData, float highAltitudeHitDistance, inout float alpha, inout float3 cloudColor)
{
    ...
    for (int a = 0; a <= MAX_STEPS; a++)
    {
        ...
        valueAtPoint *= lerp(0.6, 1.0, half(a) * INV_MAX_STEPS);
...
FragmentOutput SampleClouds(float2 UV)
{
    float alpha = 1.0;
    ...
    if (_ShadowPass == 1)
    {
        ...
        UV.x = Remap(_CascadeOffsets[cascadeIndex].x, _CascadeOffsets[cascadeIndex].y, 0, 1, UV.x);
        UV.y = Remap(_CascadeOffsets[cascadeIndex].z, _CascadeOffsets[cascadeIndex].w, 0, 1, UV.y);
        
        float2 orthoSize = _CloudShadowOrthoParams.xy * _ShadowMapCascades[cascadeIndex];
        ...
        materialData.rayOrigin = _ShadowCasterCameraPosition[cascadeIndex].xyz + _ShadowCasterCameraRight * orthoSize.x + _ShadowCasterCameraUp * orthoSize.y;
        rayData.rayOrigin = materialData.rayOrigin;
    ...
    if (sampleLowAltitudeClouds)
    {
        ...
        for (int i = 1; i <= int(_CLOUD_STEP_COUNT); i++)
        {
            ...
            if (valueAtPoint > EPSILON_LARGE && isFirstSample && _ShadowPass == 0)
            {
                i -= 1;
